...
d3.contextViewVariant = function() {
    ...
    data = [],
    svgVariant = d3.select(),
    gVariant = d3.select();
    
    function cv(selection) {
	selection.each(function({		
	    data = d;
	    svgVariant = d3.select(this).selectAll("svg").data([data]);
	...
	function drawVariantDetailsContext() 
	{
			...
			{		
				...
				if(d.transcripts.length > 0)
				{
					text.append("tspan")
						...
						.attr("y", yScale(y++))
						.attr("font-size", "12")
						.text("Which has intersected with the following gene transcript(s):");
					...
					for(var i=0; i<d.transcripts.length; i++)
					{
						...
						outStr = 
							"Name: " + d.transcripts[i].transcript + 
							", Class: " + d.transcripts[i].Class + 
							", Type: " + d.transcripts[i].loc_type + 
							", Strand: " + d.transcripts[i].strand + 
							", intron/exon distance: " + d.transcripts[i].intron_exon_dist + 
							", Variant effect: " + d.transcripts[i].variant_effect;
				...
				else
				{
					...
					text.append("tspan")
						...
						.attr("font-size", "15")
						.attr("fill", "red")
						.text("Intersecting gene transcript(s) not found.");
				}								
			});
		...
		container.append("iframe")
			...
			.attr("y", yScale(y++))
			.attr("width", "100%")
			.attr("height",  yMin + yScale(y) + 1000 + "px")
			.attr("src", function(d)
			{ 
				return d.ucscLink; 
			});			
...
d3.contextViewGene = function() 
{
	...
	function cg(selection) 
	{
		...
		{		
			...
			svgGene = d3.select(this).selectAll("svg").data([data]);
			...
			{ 					
				...
				if (typeof d.data[0].error == 'undefined')
				{
					...
					text.append("tspan")
						...
						.attr("y", yScale(y++))
						.attr("font-size", "12")
						.text("Gene description: " + d.data[0].description);
				}
				else
				{
					// output the error reason
						...
						.attr("font-size", "15")
						.attr("fill", "red")
						.text("Error getting gene details. Message: " + d.data[0].error);
				}
			});
	...
	function drawTranscriptDetailsContext(selectedTranscriptData, x, y)
	{
		...
		text = gGene.append("tspan")
			...
			.text(function(d) 
			{
				...
				if (alleleNotes["error"] == undefined && alleleNotes["data"] != undefined)
				{
					...
					{								
						...
						text.append("tspan")
							...
							.attr("y", yScale(y++))
							.attr("font-size", "12")
							.text("NCGENES notes for this allele: ");	
						...
						{																	
							...
							{								
								...
								text.append("tspan")
									...
									.attr("y", yScale(y++))
									.attr("font-size", "12")
									.text("Note " + (i+1) + ": " + alleleNotes["data"][i]["Note"]);
							}
						}						
				...
				// did we get a error
				if (typeof transcriptDesc["error"] == 'undefined')
				{
					// did we get any data
						...
						{																	
							...
							if(transcriptDesc["data"][i]["alignments"] !== undefined && transcriptDesc["data"][i]["alignments"].length > 0)
							{
								...
								text.append("tspan")
									...
									.attr("y", yScale(y++))
									.attr("font-size", "12")
									.text("Transcript mapping for accession: " + transcriptDesc["data"][i].accession);		
								...
								for(var j=0; j<tMapData.length; j++)
								{
									...
									outStr = 
										CheckNull("Genomic accession: ", tMapData[j].genomicAccession) +
										CheckNull("Strand type: ", tMapData[j].strandType) + 
										CheckNull(", Genomic start: ", tMapData[j].genomicStart) + 
									...
									tMapData[j]["regions"].sort(function(a, b) { return d3.ascending(a.regionStart, b.regionStart); });
									...
									for(var k=0; k<tMapData[j]["regions"].length; k++)
									{
										...
										outStr = 
											CheckNull("Region Type: ", rData.regionType) +
											CheckNull(", Region start: ", rData.regionStart) + 
											CheckNull(", Region stop: ", rData.regionStop) + 
											CheckNull(", cds start: ", rData.cdsStart) + 
											CheckNull(", cds stop: ", rData.cdsStop) + 
											CheckNull(", Transcript start: ", rData.transcriptStart) + 
					...
					else
					{
						...
						text.append("tspan")
							...
							.attr("font-size", "15")
							.attr("fill", "red")
							.text("Error getting transcript details. Message: No data returned");
					}
				}
				else
				{
					...
					.attr("font-size", "15")
					.attr("fill", "red")
					.text("Error getting transcript details. Message: " + transcriptDesc["error"]);
				}
