...
{		
	...
	try
	{
		...
		httpreq.open("POST", "Utils/AjaxData.php", false);
		...
		// send all relevant parameters to AjaxUtil.php.  See that file for more details
		httpreq.send(sendParams);
				
		// did we get anything. length check arbitrarily set to 5 because all returns should at lease start with "{" plus "data" or "error"...
		...
		{
			errorDisplay("launchDataRequest() - No data. Type: " + functionName + "()");
		}
	}
	catch(e)
	{
